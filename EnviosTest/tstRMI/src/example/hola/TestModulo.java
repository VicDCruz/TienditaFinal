...
public class TestModulo {
    ...
    private static final int TOTALSERVICES = 5;
    ...
    public static double doRequest() {
        example.hi.CalculatorCAService1 service = new example.hi.CalculatorCAService1();
        ...
        switch (getOperation()) {
            ...
            case 5: {
                try {
                    output = moduloOperation(input, port);
                } catch (example.hi.OperationFault ex) {
                    System.out.println(ex.getFaultInfo().getFaultDetail());
                    return 0;
                }
    ...
    public static void main(String args[]) {
        for (int i = 0; i < 10; i++) {
            System.out.println("Resultado " + TestModulo.doRequest());
        }
    }
    ...
    private static example.hi.OutputComplexType moduloOperation(
            example.hi.InputComplexType part1,
            example.hi.MyCalculatorPortType port) throws example.hi.OperationFault {
        return port.moduloOperation(part1);
    }
